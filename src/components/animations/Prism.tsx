...
const Prism: React.FC<PrismProps> = ({
  ...
  return (
    <div className="w-full h-screen relative" ref={containerRef}>
      <div className="py-2 pb-10 bg-no-repeat h-full  bg-cover bg-center text-white">
        <div className="w-full relative z-50 md:w-[90vw] mx-auto">
          <Header />
        </div>

        <div className="flex relative z-50 items-center h-full justify-center gap-5 lg:gap-8 xl:gap-10 px-5 sm:px-8 md:px-10 xl:px-16 mt-28 lg:mt-16 xl:mt-24">
          {/* Center Text */}
          <div className="basis-[50%] text-white xl:basis-[40%] flex-grow flex flex-col items-center gap-0 md:gap-2">
            <p className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-7xl text-center font-medium font-Bricolage_Grotesque overflow-hidden">
              Tax Consultancy Services
            </p>
            <p className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-medium text-center font-Bricolage_Grotesque overflow-hidden">
              {/* Consultancy Services */}
            </p>
            <div className="text-sm mt-3 sm:mt-0 sm:w-3/4 text-center font-Bricolage_Grotesque">
              <p>
                At Auditique Consulting, we provide expert Tax and Business Consultancy
                Services to minimize your taxes and grow
              </p>
            </div>

            <div className="relative xl:w-1/2 mt-5">
              ...
              {/* <div
                className={`absolute left-0 right-0 mt-2 bg-white rounded-xl shadow-lg z-10 overflow-hidden transition-all duration-500 ease-in-out
                  ${query ? "opacity-100 scale-y-100 h-fit max-h-28 scrollbar-hide overflow-y-auto pointer-events-auto" : "opacity-0 scale-y-0 pointer-events-none"}
                `}
              >
                {filteredServices.length > 0 ? (
                  filteredServices.map((service, index) => (
                    <Link key={index} href={`/services/${service.pathName}`}>
                      <div className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-black">{service.label}</div>
                    </Link>
                  ))
                ) : (
                  <div className="px-4 py-2 text-gray-500">No service found</div>
                )}
              </div> */}
            </div>
          </div>
