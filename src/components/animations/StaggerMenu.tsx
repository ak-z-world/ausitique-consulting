...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div className="sm-scope md:hidden w-full h-full fixed mx-auto">
      <div
        className={
          (className ? className + " " : "") +
      ...
      >
        ...
        {/* <header
          className={` fixed top-2 ${!open && 'bg-black/40 backdrop-blur-3xl'} left-0 w-[95%] mx-auto flex items-center justify-between px-4 py-2  rounded-4xl  pointer-events-none z-20`}
          aria-label="Main navigation header"
        > */}
        <header
          className={`fixed top-2 left-1/2 -translate-x-1/2 ${
            !open && "bg-black/60 backdrop-blur-2xl"
          } 
              w-[95%] flex items-center justify-between px-4 py-2 
              rounded-4xl pointer-events-none z-20`}
          aria-label="Main navigation header"
        >
          <Link href='/'>
          <div
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white flex flex-col p-[6em_1em_2em_1em] overflow-y-auto z-10 backdrop-blur-[12px]"
          style={{ WebkitBackdropFilter: "blur(12px)" }}
          aria-hidden={!open}
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            <ul
              className=" list-none m-0 p-0 flex flex-col gap-5"
              role="list"
...
.sm-scope .sm-socials-link { font-size: 1rem; font-weight: 500; color: #111; text-decoration: none; position: relative; padding: 2px 0; display: inline-block; transition: color 0.3s ease, opacity 0.3s ease; }
